...
test_that("pvalue in one stratum", {
  ...
  expect_equal(
    RIQITE::pval_quantile(Z = Z, Y = Y, k = k, c = c, method.list = method.list.old.wil),
    RIQITE::pval_comb(Z = Z, Y = Y, k = k, c = c,
                      block = block, methods.list.all = method.lists.wil, statistic = FALSE),
    tolerance = 0.001)

  expect_equal(
    RIQITE::pval_quantile(Z = Z, Y = Y, k = k, c = c, method.list = method.list.old.ste),
    RIQITE::pval_comb(Z = Z, Y = Y, k = k, c = c,
                      block = block, methods.list.all = method.lists.ste, statistic = FALSE),
    tolerance = 0.001)
})
